<x-guest-layout>
    ...
    <form class="flex flex-col gap-3"  action="{{route('incidents.store')}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        @php
            $now = date("Y-m-d");
        @endphp
        <input type="date" name="dateTime" id="dateTime" max="{{$now}}"  value="{{ old('dateTime', '') }}">
        @error('dateTime')
